...
// ── PERSISTENCE: /tmp survives warm Vercel instances ──────────────────────
const DB_FILE = '/tmp/kvl_db.json';
...
// ── Load DB from /tmp (persists across warm Vercel invocations) ─────────────
function loadDB() {
  try {
    if (fs.existsSync(DB_FILE)) {
      const raw = fs.readFileSync(DB_FILE, 'utf8');
      const saved = JSON.parse(raw);
      // Credentials are immutable from code — always use code version for auth
      saved.credentials = DEFAULT_DB.credentials;
      console.log('[KVL] Loaded DB from /tmp, saved at:', saved._savedAt);
      return saved;
    }
  } catch(e) {
    console.warn('[KVL] Could not load /tmp DB:', e.message);
  }
  console.log('[KVL] Using DEFAULT DB (fresh start)');
  return JSON.parse(JSON.stringify(DEFAULT_DB));
}

function persistDB() {
  try {
    global._DB._savedAt = new Date().toISOString();
    fs.writeFileSync(DB_FILE, JSON.stringify(global._DB));
  } catch(e) {
    console.warn('[KVL] Could not persist DB to /tmp:', e.message);
  }
}
...
// ── Health ──────────────────────────────────────────────────────────────────
app.get('/api/health',(req,res)=>res.json({ok:true,ts:new Date().toISOString(),savedAt:DB._savedAt}));
...
app.get('/api/state',auth,(req,res)=>{
  res.json({
    ...
    currentWeek:DB.currentWeek,actST:DB.actST,
    credentials:req.user.role==='admin'?DB.credentials:[],
    _savedAt:DB._savedAt
  });
});

// ── POST state — admin pushes full or partial state ─────────────────────────
app.post('/api/state',auth,adm,(req,res)=>{
  ...
  persistDB(); // ← Write to /tmp immediately after every state change
  res.json({ok:true,savedAt:DB._savedAt});
});
