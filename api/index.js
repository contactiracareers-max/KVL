...
app.post('/api/submit',auth,(req,res)=>{
  ...
  // Store full submission — participant, tCo, tPart, type label all included from client
  const sub={
    ...req.body,
    id: uuidv4(),
    by: req.user.id,
    byName: req.user.name||req.user.u,
    status: 'pending',
    submittedAt: new Date().toISOString(),
    date: new Date().toLocaleString('en-IN')
  };
  // Ensure participant name falls back to login name if not sent
  if(!sub.participant||sub.participant==='—') sub.participant=req.user.name||req.user.u;
