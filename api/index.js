...
// ══════════════════════════════════════════════════════════════════
// PERMANENT STORAGE — JSONBin.io (hardcoded — no env vars needed)
// FIX: submissions have base64 photos → strip before saving to JSONBin
//      to stay under 1MB limit. Photos are ephemeral (session-only).
//      All scores, valuations, credentials, setups persist forever.
// ══════════════════════════════════════════════════════════════════
...
// Strip photos from submissions/gallery before saving to JSONBin
// Photos are base64 strings — can be 100KB each, blowing past 1MB limit
function stripPhotos(submissions) {
  return (submissions || []).map(s => {
    const copy = {...s};
    if (copy.photo) delete copy.photo; // remove base64 — ephemeral
    return copy;
  });
}
...
async function saveToJSONBin(state) {
  const toSave = {
    ...
    setups: state.setups,
    submissions: stripPhotos(state.submissions), // ← strip photos — key fix
    weekMultipliers: state.weekMultipliers,
    actST: state.actST,
    ledger: (state.ledger || []).slice(-200), // keep last 200 ledger entries
    dissolved: state.dissolved || [],
    news: (state.news || []).slice(0, 30),
    gallery: [], // gallery has photos — don't store in JSONBin
  ...
  console.log(`JSONBin save: ${sizeKB.toFixed(1)}KB`);
  if (sizeKB > 900) {
    console.log('⚠️  Payload too large — truncating submissions');
    toSave.submissions = toSave.submissions.slice(-50); // keep last 50
  }
  try {
    ...
    if (r.status === 200) console.log('✅ JSONBin saved OK');
    else console.log('❌ JSONBin error:', r.status, JSON.stringify(r.body).slice(0, 150));
...
async function loadFromJSONBin() {
  try {
    ...
    console.log('JSONBin load status:', r.status);
  } catch(e) { console.log('JSONBin load error:', e.message); }
  return null;
}

// ── DEFAULT DATA ──────────────────────────────────────────────────
const KEBI = 'KEBI-Kohinoor Exchange Board of India';
const DC = [
  ...
  {id:9,name:KEBI,ticker:'KEBI',val:0,chg:0,h:[0],acts:{p2p:0,referral:0,business:0,visitor:0,board:0,joint:0,stakeholder:0}}
];
const DST = [
  {id:'L1',name:'Level 1 — Foundation',criteria:[
    ...
    {id:'business',lbl:'Business Given',pts:150,on:true},{id:'visitor',lbl:'Visitor Invited',pts:40,on:true},
  ...
  {id:'L2',name:'Level 2 — Advanced',criteria:[
    ...
    {id:'business',lbl:'Business Given',pts:250,on:true},{id:'visitor',lbl:'Visitor Invited',pts:60,on:true},
...
const DP = [
  ...
  {co:KEBI,name:'Harshit Shah',pos:'KEBI Admin'},{co:KEBI,name:'Nikesh Shah',pos:'KEBI Admin'},
  {co:KEBI,name:'Rupesh Shah',pos:'KEBI Admin'},{co:KEBI,name:'Kaushal Shah',pos:'KEBI Admin'},
  {co:KEBI,name:'Pratik Shah',pos:'KEBI Admin'}
];
const DT = [
  ...
  {id:9,name:KEBI,color:'#FFD700',members:[{name:'Harshit Shah'},{name:'Nikesh Shah'},{name:'Rupesh Shah'},{name:'Kaushal Shah'},{name:'Pratik Shah'}]}
];
const DCR = [
  {id:'harshit_s',u:'harshit.s',p:'harkebi@1',role:'admin',co:KEBI,name:'Harshit Shah',pos:'KEBI Admin'},
  {id:'nikesh_s',u:'nikesh.s',p:'nikkebi@2',role:'admin',co:KEBI,name:'Nikesh Shah',pos:'KEBI Admin'},
  {id:'rupesh_s',u:'rupesh.s',p:'rupkebi@3',role:'admin',co:KEBI,name:'Rupesh Shah',pos:'KEBI Admin'},
  {id:'kaushal_s',u:'kaushal.s',p:'kaukebi@4',role:'admin',co:KEBI,name:'Kaushal Shah',pos:'KEBI Admin'},
  {id:'pratik_p',u:'pratik.p',p:'prakebi@5',role:'admin',co:KEBI,name:'Pratik Shah',pos:'KEBI Admin'},
...
// On cold start: load from JSONBin ONCE
let _loadPromise = null;
function ensureLoaded() {
  if (DB.seeded) return Promise.resolve();
  if (!_loadPromise) {
    _loadPromise = loadFromJSONBin().then(saved => {
      if (saved) {
        ...
        console.log('Loaded from JSONBin OK');
      } else {
        console.log('JSONBin empty — using defaults');
      }
...
app.post('/api/state',auth,adm,(req,res)=>{
  ...
  if(submissions!=null){
    if(submissions.length>=DB.submissions.length)DB.submissions=submissions;
    else{const ids=new Set(DB.submissions.map(s=>s.id));const n=submissions.filter(s=>!ids.has(s.id));if(n.length)DB.submissions=[...DB.submissions,...n];}
  }
  if(gallery!=null){
    if(gallery.length>=(DB.gallery||[]).length)DB.gallery=gallery;
    else{const ids=new Set((DB.gallery||[]).map(g=>g.photo));const n=gallery.filter(g=>!ids.has(g.photo));if(n.length)DB.gallery=[...(DB.gallery||[]),...n];}
  }
  if(ledger!=null){
    if(ledger.length>=(DB.ledger||[]).length)DB.ledger=ledger;
    else{const ids=new Set((DB.ledger||[]).map(l=>l.id));const n=ledger.filter(l=>!ids.has(l.id));if(n.length)DB.ledger=[...(DB.ledger||[]),...n];}
  }
  ...
  if(credentials&&credentials.length){
    const admins=DB.credentials.filter(c=>c.role==='admin');
    DB.credentials=[...admins,...credentials.filter(c=>c.role!=='admin')];
  }
  DB.seeded=true; DB.savedAt=new Date().toISOString();
  saveToJSONBin(DB); // async — photos stripped, size-checked
...
app.post('/api/submit',auth,(req,res)=>{
  ...
  // Build complete submission record — all fields for participant tracking
  const sub = {
    ...req.body,
    id:        uuidv4(),
    by:        req.user.id,   // credential id e.g. "atit_s"
    byU:       req.user.u,    // username e.g. "atit.s"
    byName:    req.user.name||req.user.u,  // display name
    status:    'pending',
    submittedAt: new Date().toISOString(),
    date:      new Date().toLocaleString('en-IN')
  };
  // Always use login name as participant if not provided
  if(!sub.participant||sub.participant==='—'||sub.participant==='No participants'){
    sub.participant = req.user.name||req.user.u;
  }
  // Ensure co matches login company for participants
  if(req.user.role==='participant'&&req.user.co){
    sub.co = req.user.co;
  }
  ...
  DB.news.unshift({t:'🔔 '+sub.participant+' ('+sub.co+') submitted '+(sub.type||sub.tId)+' — pending approval',up:true});
...
app.post('/api/approve/:id',auth,adm,(req,res)=>{
  ...
  if(co){const old=co.val||0;co.val=Math.max(0,old+pts);co.chg=old>0?parseFloat(((co.val-old)/old*100).toFixed(1)):0;co.h.push(co.val);if(co.h.length>20)co.h.shift();}
...
app.post('/api/reject/:id',auth,adm,(req,res)=>{
  ...
  if(!sub)return res.status(404).json({error:'Not found'});
  sub.status='rejected';
  DB.news.unshift({t:`❌ ${sub.co} — ${sub.type} REJECTED · ${sub.participant||''}`,up:false});
  if(DB.news.length>30)DB.news=DB.news.slice(0,30);
  DB.savedAt=new Date().toISOString();
  saveToJSONBin(DB);
  res.json({ok:true});
...
app.post('/api/credentials',auth,adm,(req,res)=>{
  ...
  const c={...req.body,id:req.body.id||uuidv4()};
