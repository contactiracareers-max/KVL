...
// ── PERSISTENCE: /tmp/kvl_state.json survives warm instances ──────────────
// On cold start: load from file if exists, else use defaults
// On every state change: write to file immediately
const STATE_FILE = '/tmp/kvl_state.json';
...
// ── Load or init DB ──────────────────────────────────────────────────────────
function loadDB() {
  // 1. Try in-memory (same warm instance)
  if (global._DB && global._DB.seeded) return global._DB;
  // 2. Try file (survived a redeploy/restart within same Vercel instance lifetime)
  try {
    const raw = fs.readFileSync(STATE_FILE, 'utf8');
    const saved = JSON.parse(raw);
    if (saved && saved.seeded) {
      global._DB = saved;
      // Always keep credentials from code (never store in file — security)
      global._DB.credentials = DEFAULT_CREDS;
      console.log('Loaded state from file, savedAt:', saved.savedAt);
      return global._DB;
    }
...
function persistDB() {
  try {
    // Don't store credentials in file
    const toSave = {...global._DB, credentials: []};
...
// POST state — only admin can push; always accepted from admin (authoritative)
app.post('/api/state',auth,adm,(req,res)=>{
  const{companies,teams,participants,setups,submissions,gallery,ledger,
        dissolved,weekMultipliers,news,actST}=req.body;
  ...
  if(submissions!=null){
    // Never shrink — merge to keep all approved data
  ...
  // Allow restoring participant credentials from backup (admin creds stay hardcoded)
  const{credentials}=req.body;
  if(credentials&&credentials.length){
    const admins=DB.credentials.filter(c=>c.role==='admin');
    const parts=credentials.filter(c=>c.role!=='admin');
    DB.credentials=[...admins,...parts];
  }
  DB.seeded=true;
...
app.post('/api/reject/:id',auth,adm,(req,res)=>{
  const sub=DB.submissions.find(s=>s.id===req.params.id);
  if(!sub)return res.status(404).json({error:'Not found'});
  sub.status='rejected';DB.savedAt=new Date().toISOString();persistDB();
  res.json({ok:true});
});
...
app.delete('/api/credentials/:id',auth,adm,(req,res)=>{
  ...
  DB.credentials=DB.credentials.filter(x=>x.id!==req.params.id);persistDB();res.json({ok:true});
});
